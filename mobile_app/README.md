...
1. 開啟應用程式後，您會看到登入與設定介面。
2. 依序輸入以下資訊：
   - **帳號** (您的 Portal 學號)
   - **密碼** (您的 Portal 密碼)
   - **一般課程** (每行一個課程，格式為 `系所代碼,課程代碼+班級`，適用於希望全部選上的課程)
   - **同時段群組** (可點擊 **「新增同時段群組」** 按鈕來建立群組。在群組輸入框中每行填入一個課程。此群組內的課程為「擇一選上」，只要選上其中一門，同群組剩餘的課程即會被自動跳過)
   - **延遲時間** (選填，預設為 2.5 秒)
3. 點擊 **「開始選課」**，系統將自動於背景執行選課流程。
4. 下方的 **「選課狀態」** 與 **「執行記錄」** 區塊會顯示即時的狀態與詳細日誌。
5. 若要中斷，請點擊 **「停止」**。
